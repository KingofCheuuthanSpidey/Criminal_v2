...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅ Criminal_v2 working!')
        );
...
*/
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ Criminal ~______~*' +
                                    '\n\n*🧞‍♂️ ' + error + '*\n\n```Report errors\njoin with +91 \https://chat.whatsapp.com/JRdGiHfLk1rBgAuBCS5BUn``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
